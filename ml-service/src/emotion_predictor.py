...
class EmotionPredictor:
    """Predicts facial emotions using trained PyTorch CNN model."""

    def __init__(self, model_path=None):
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        if model_path is None:
            for name in ['emotion_model_best.pth', 'emotion_model.pth']:
        ...
        if model_path is None or not Path(model_path).exists():
            raise FileNotFoundError("Model not found. Train first with train_emotion.py")

        print(f"📦 Loading emotion model from {model_path}...")

        # Always reconstruct model and load state dict
        from train_emotion import EmotionCNN
        self.model = EmotionCNN()
        state = torch.load(model_path, map_location=self.device, weights_only=True)
        # Handle if it's a full model save vs state_dict
        if isinstance(state, dict) and 'block1.0.weight' in state:
